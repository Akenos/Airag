$(document).ready(function () {
  ...
  $(document).on("keypress", function () {
    if (!gameStarted && !secondRound) { // equivalient of gameStarted = false
      gameStarted = true;
      startNextLevel();
    };
    if(!gameStarted && secondRound){
      gameStarted = true;
      startNextLevel();
  ...
  function startNextLevel() {
    ...
    userSequence = [];
    if(levelNumber === MaxLevel && !secondRound) {
      $("#level-title").text("You won the game! if you want to continue the game in next Round Press Any BUTTON ");
      gameStarted = false;
      secondRound = true;
      sequenceMemory = [];
      userSequence = [];
    ...
    if(secondRound){
      levelNumber++;
      $("#level-title").text("Second Round: Level " + levelNumber);
      sequenceMemory = [];
      for(var i = 0 ; i < levelNumber; i++){
        ...
        }, i * 1000); // for some reasion without i * 1000, all the of them simulstaneyl flashes
      };
